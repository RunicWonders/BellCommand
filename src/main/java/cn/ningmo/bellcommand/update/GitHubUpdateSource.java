...
public class GitHubUpdateSource implements UpdateSource {
    ...
    public GitHubUpdateSource(BellCommand plugin) {
        this.plugin = plugin;
        ConfigurationSection config = plugin.getConfig().getConfigurationSection("update-source.github");
        if (config != null) {
            ...
            if (plugin.isDebugEnabled()) {
                plugin.getLogger().warning("GitHub配置部分缺失，使用默认值");
            }
        }
    }
